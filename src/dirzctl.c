...
command_config_dirzctl(uint32_t *args)
{
    sendf("debug_dirzctl oid=%c arg[0]=%u arg[1]=%u"
          ...
          (uint8_t)args[0], (uint32_t)args[0],
          (uint32_t)args[1], (uint32_t)args[2],
          (uint32_t)args[3], (uint32_t)args[4],
          (uint32_t)args[5]);
...
command_run_dirzctl(uint32_t *args)
{
    ...
    d->n_ticks = (uint32_t)(
        ((double)args[2] / 1000000) * CONFIG_CLOCK_FREQ) / 2;
    d->r_steps = args[3] * 2;
...
DECL_COMMAND(command_run_dirzctl,
             "run_dirzctl oid=%c direct=%c"
             " step_us=%u step_cnt=%u");
...
dirzctl_task(void)
{
    ...
    foreach_oid(oid,d,command_config_dirzctl)
    {
        ...
        sendf("result_dirzctl oid=%c step=%u tick=%u",
              (uint8_t)d->oid, (uint16_t)d->a_steps,
              (uint32_t)d->a_ticks);
