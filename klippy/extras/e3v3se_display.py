...
class E3V3SEPrinterSerialBridge(PrinterSerialBridge):
    ...
    customize initialization without modifying the
    original PrinterSerialBridge class.

    This class defaults the serial pins, baud, eol and
    serial bridge config to specific
...
class E3v3seDisplayMacro:
    def __init__(self, config):
        ...
        parent.log(
            "E3V3SE Display MACRO:"
            " id: '%s' label: '%s' icon: '%i' gcode: '%s'" %
            (id, label, icon, gcode))
...
class E3v3seDisplay:
    ...
    def handle_ready(self):
        ...
        try:
            ...
            if (self.display_status is None and
                    "M117" not in self.gcode.ready_gcode_handlers):
    ...
    def HMI_StartFrame(self, with_update):
        ...
        if self.pd.status == "printing":
            ...
            self.Draw_Status_Area(with_update)
                ...
                "operational", "complete", "standby", "cancelled"]:
            self.Goto_MainMenu()
        else:
            self.Goto_MainMenu()
    ...
    def HMI_MainMenu(self):
        ...
        if encoder_state == self.ENCODER_DIFF_NO:
            return
        if (encoder_state == self.ENCODER_DIFF_CW or
    ...
    def HMI_SelectFile(self):
        ...
        if (encoder_state == self.ENCODER_DIFF_CW or encoder_state ==
            ...
            if self.select_file.inc(1 + fullCnt):
               ...
               encoder_state == self.ENCODER_DIFF_FAST_CCW) and
              fullCnt):
    ...
    def HMI_Misc(self):
        ...
        if (encoder_state == self.ENCODER_DIFF_CW or encoder_state ==
            ...
            if self.select_misc.inc(1 + custom_macro_count):
                ...
                if (self.select_misc.now > self.MROWS and
                        self.select_misc.now > self.index_misc):
                    ...
                    self.Move_Highlight(
                        1,
                        self.select_misc.now +
                        self.MROWS -
                        self.index_misc)  # Move highlight
        elif ((encoder_state == self.ENCODER_DIFF_CCW or
               encoder_state == self.ENCODER_DIFF_FAST_CCW) and
              custom_macro_count):
    ...
    def HMI_Prepare(self):
        """
        This function handles the logic for scrolling
        through the prepare menu options,
        selecting different actions, and executing the
    ...
    def HMI_PrintPreview(self):
        ...
        if encoder_state == self.ENCODER_DIFF_NO:
            return
        elif (encoder_state == self.ENCODER_DIFF_CW or
                encoder_state == self.ENCODER_DIFF_FAST_CW):
    ...
    def HMI_AxisMove(self):
        ...
        This function checks the encoder state and performs
        the corresponding action based on the state.
        It updates the display and handles the movement of
    ...
    def HMI_Temperature(self):
        ...
        if (encoder_state == self.ENCODER_DIFF_CW or
            ...
            # Nozzle temperature
                ...
                hotend = self.pd.thermalManager["temp_hotend"]
                self.pd.HMI_ValueStruct.E_Temp = hotend[0]["target"]
                self.lcd.draw_int_value(
                    True,
                    True,
                    0,
                ...
                bed = self.pd.thermalManager["temp_bed"]
                self.pd.HMI_ValueStruct.Bed_Temp = bed["target"]
                self.lcd.draw_int_value(
                    True,
                    True,
                    0,
                ...
                fan = self.pd.thermalManager["fan_speed"]
                self.pd.HMI_ValueStruct.Fan_speed = fan[0]
                self.lcd.draw_int_value(
                    True,
                    True,
                    0,
            ...
            # PLA preheat setting
            elif self.select_temp.now == self.TEMP_CASE_PLA:
            ...
            # TPU preheat setting
            elif self.select_temp.now == self.TEMP_CASE_TPU:
    ...
    def HMI_PLAPreheatSetting(self):
        ...
        # Avoid flicker by updating only the previous menu
            ...
            if self.select_PLA.now == 0:  # Back
                ...
                self.Draw_Temperature_Menu()
            # Nozzle temperature
            elif self.select_PLA.now == self.PREHEAT_CASE_TEMP:
                self.checkkey = self.ETemp
                ...
                self.EncoderRateLimit = False
            # Bed temperature
            elif self.select_PLA.now == self.PREHEAT_CASE_BED:
                self.checkkey = self.BedTemp
    ...
    def HMI_TPUPreheatSetting(self):
        ...
        # Avoid flicker by updating only the previous menu
            ...
            # Nozzle temperature
            elif self.select_TPU.now == self.PREHEAT_CASE_TEMP:
                self.checkkey = self.ETemp
                self.pd.HMI_ValueStruct.E_Temp = (
                    self.pd.material_preset[1].hotend_temp)
                ...
                self.EncoderRateLimit = False
            # Bed temperature
            elif self.select_TPU.now == self.PREHEAT_CASE_BED:
                self.checkkey = self.BedTemp
                self.pd.HMI_ValueStruct.Bed_Temp = (
                    self.pd.material_preset[1].bed_temp)
                self.lcd.draw_int_value(
    ...
    def HMI_Zoffset(self):
        ...
        if encoder_state == self.ENCODER_DIFF_ENTER:
            ...
            self.checkkey = (
                self.Prepare if self.pd.HMI_ValueStruct.show_mode == -
                4 else self.Tune)
            self.lcd.draw_signed_float(
                True,
                self.lcd.font_8x8,
    ...
    def Draw_Status_Area(self, with_update=True):
        ...
        self.lcd.draw_int_value(
            ...
            self.lcd.font_8x8,
            self.color_yellow
            if self.pd.nozzleIsHeating() else self.color_white,
            self.color_background_black,
            3,
            26,
            268,
    ...
    def Draw_Print_Preview(self):
        ...
        self.lcd.draw_string(
            ...
            if self.pd.metadata['estimated_time'] else "N/A")
        ...
        self.lcd.draw_string(
            ...
            if self.pd.metadata['filament_used'] else "N/A")
        ...
        self.lcd.draw_string(
            ...
            if self.pd.metadata['layer_height'] else 'N/A')
    ...
    def Draw_Tune_Menu(self):
        ...
        if self.pd.HAS_HOTEND:
            ...
            self.lcd.move_screen_area(
                ...
                self.LBLX + 44,
                self.MBASE(
                    self.TUNE_CASE_TEMP))  # Temperature
        if self.pd.HAS_HEATED_BED:
            self.lcd.move_screen_area(
            ...
            self.lcd.move_screen_area(
                ...
                self.LBLX + 27,
                self.MBASE(
                    self.TUNE_CASE_BED))  # ...Temperature
        if self.pd.HAS_FAN:
            self.lcd.move_screen_area(
    ...
    def Draw_Move_Menu(self):
        ...
        This method clears the main window and then
        proceeds to draw the "Move" header.
        It also draws the options for moving the X, Y,
        and Z axes, as well as the extruder
        option if available. It also handles drawing the
    ...
    def Draw_Manual_Probe_Menu(
            ...
            draw_error=False):
        if (self.manual_probe is None or
                self.manual_probe.status["is_active"] == False):
            self.Goto_MainMenu()
            return
    ...
    def Popup_Window_ETempTooLow(self):
        """
        Displays a popup window indicating that the
    ...
    def show_popup(self, message=""):
        try:
            ...
            # firmware_restart is called and this tries
    ...
    def CompletedHoming(self):
        ...
        if self.checkkey == self.Last_Prepare:
            ...
            self.pd.feedrate_percentage = 100
            self.pd.HMI_ValueStruct.print_speed = 100
    ...
    def EachMomentUpdate(self, eventtime):
        ...
        if self.checkkey == self.PrintProcess:
            if (self.pd.HMI_flag.print_finish and not
                    self.pd.HMI_flag.done_confirm_flag):
                self.pd.HMI_flag.print_finish = False
        ...
        if (self.display_status is not None and
                self.display_status.message and
                len(self.display_status.message) > 0 and
                self.last_display_status !=
