...
class PRTouchZOffsetWrapper:
    ...
    def ck_sys_sta(self):
        return (not self.obj.hx711s.is_shutdown and
                not self.obj.hx711s.is_timeout and
                not self.obj.dirzctl.is_shutdown and
                not self.obj.dirzctl.is_timeout)
    ...
    def _move(self, pos, speed, wait=True):
        if (not self.obj.hx711s.is_shutdown and
                not self.obj.hx711s.is_timeout and
                not self.obj.dirzctl.is_shutdown and
                not self.obj.dirzctl.is_timeout):
    ...
    def _check_trigger(
        ...
        if (len(fit_vals) >= (self.cfg.pi_count / 2) and
                math.fabs(fit_vals[-1]) >= max_hold and
                math.fabs(fit_vals[-2]) >= max_hold and
                math.fabs(fit_vals[-3]) >= max_hold):
        ...
        if (self.val.out_index > 0):
            for i in range(self.val.out_index, self.cfg.pi_count):
                ...
                idx = self.val.out_index
                fit_vals_t[idx] = (
                    fit_vals_t[idx] * k +
                    fit_vals_t[idx - 1] * (1 - k))
        vals_p = [x for x in fit_vals_t]
    ...
    def _set_hot_temps(self, temp, fan_spd, wait=False, err=5):
        ...
        # self.obj.gcode.run_script_from_command(
        #     'SET_PIN PIN=fan0 VALUE=%d' % (fan_spd))
        # self.obj.gcode.run_script_from_command(
        #     'SET_PIN PIN=fan1 VALUE=%d' % (fan_spd))
        # self.obj.gcode.run_script_from_command(
        #     'SET_PIN PIN=fan2 VALUE=%d' % (fan_spd))
        if wait:
            while (self.ck_sys_sta() and
                   abs(self.obj.heater_hot.target_temp -
                       self.obj.heater_hot.smoothed_temp) > err and
                   self.obj.heater_hot.target_temp > 0):
    ...
    def _set_bed_temps(self, temp, wait=False, err=5):
        ...
        if wait:
            while (self.ck_sys_sta() and
                   abs(self.obj.heater_bed.target_temp -
                       self.obj.heater_bed.smoothed_temp) > err and
                   self.obj.heater_bed.target_temp > 0):
                self.obj.hx711s.delay_s(0.100)
        pass
    ...
    def _cal_min_z(self, start_z, hx711_vals):
        ...
        if dirzctl_params is None or len(dirzctl_params) != 2:
            raise self.obj.printer.command_error(
                '{"code":"key502", "msg":'
                '"probe_by_step: Can not recv stepper-z status."}')
        ...
        tick0 = dirzctl_params[0]['tick']
        dirzctl_params[0]['tick'] = (
            (tick0 - dirzctl_start_tick) /
            self.obj.dirzctl.mcu_freq)
        wrap1 = (4294967296
                 if dirzctl_params[1]['tick'] < dirzctl_start_tick
                 else 0)
        dirzctl_params[1]['tick'] = (
            (wrap1 + dirzctl_params[1]['tick'] - dirzctl_start_tick) /
            self.obj.dirzctl.mcu_freq)
        dirzctl_params[0]['z'] = start_z
        steps = (dirzctl_params[0]['step'] -
                 dirzctl_params[1]['step'] + 1)
        step_dist = (self.obj.dirzctl.steppers[0].get_step_dist() *
                     self.obj.dirzctl.step_base)
        dirzctl_params[1]['z'] = start_z - steps * step_dist
        oi = self.val.out_index
        nt = hx711_params[oi]['nt']
        wrap_h = 4294967296 if nt < hx711_start_tick else 0
        tick_p = (wrap_h + nt - hx711_start_tick) / self.obj.hx711s.mcu_freq
        ...
        up_min_cnt = int((self.val.out_val_mm -
                          dirzctl_params[1]['z']) /
    ...
    def probe_by_step(
        ...
        while self.ck_sys_sta():
            ...
            unfit_vals, tmp_unfit_vals = self.obj.filter.cal_offset_by_vals(
                self.obj.hx711s.s_count, all_valss,
                self.obj.filter.lft_k1_oft, self.cfg.pi_count)
            fit_vals, tmp_fit_vals = self.obj.filter.cal_filter_by_vals(
                self.obj.hx711s.s_count, all_valss,
                self.obj.filter.hft_hz,
                self.obj.filter.lft_k1, self.cfg.pi_count)
            ...
            for i in range(self.obj.hx711s.s_count):
                ...
                hx711_vals, tmp_hx711_vs = self.obj.filter.cal_filter_by_vals(
                    self.obj.hx711s.s_count, all_valss,
                    self.obj.filter.hft_hz,
                    self.obj.filter.lft_k1_cal, self.cfg.pi_count)
                self.pnt_array(
                    'WAIT_AND_CAL_CH=%d, ARY=' %
                    (i), tmp_hx711_vs[i])
    ...
    def cmd_PRTOUCH_PROBE_ZOFFSET(self, gcmd):
        ...
        self.pnt_msg(
            "Checking z-position of probe (%.2f, %.2f)" %
            (probe_x, probe_y))
