...
class DirZCtl:
    ...
    def _build_config(self):
        ...
        for i in range(len(self.steppers)):
            s = self.steppers[i]
            dir_pin, step_pin, ivt_dir, ivt_step = s.get_pin_info()
            self.mcu.add_config_cmd(
                "add_dirzctl oid=%d index=%d"
                " dir_pin=%s step_pin=%s"
                " dir_invert=%d step_invert=%d" %
                (self.oid, i, dir_pin, step_pin, ivt_dir, ivt_step))

        # self.run_cmd = self.mcu.lookup_command(
        #     "run_dirzctl oid=%c direct=%c step_us=%u"
        #     " step_cnt=%u is_ck_con=%c", cq=None)
        self.run_cmd = self.mcu.lookup_command(
    ...
    def check_and_run(self, direct, step_us, step_cnt,
        ...
        if step_cnt != 0:
            self.all_params = []
        # self.run_cmd.send([
        #     self.oid, direct, step_us, step_cnt,
        #     1 if is_ck_con else 0])
    ...
    def cmd_DIRZCTL(self, gcmd):
        index = gcmd.get_int(
            'INDEX', len(
                self.steppers), minval=0, maxval=len(
                self.steppers))
