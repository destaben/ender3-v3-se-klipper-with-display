...
class HX711S:
    def __init__(self, config):
        ...
        for i in range(self.s_count):
            default_clk = None if i == 0 else self.s_clk_pin[i - 1]
            self.s_clk_pin.append(config.get(
                'sensor%d_clk_pin' % i, default_clk))
            default_sdo = None if i == 0 else self.s_sdo_pin[i - 1]
            self.s_sdo_pin.append(config.get(
                'sensor%d_sdo_pin' % i, default_sdo))
    ...
    def _build_config(self):
        self.mcu.add_config_cmd(
            "config_hx711s oid=%d hx711_count=%d" %
            (self.oid, self.s_count))
    ...
    def _handle_result_hx711s(self, params):
        ...
        if (self.del_dirty and
                (params['vd'] != 0 or params['it'] > 20) and
                self.index_dirty == 0):
            self.index_dirty = 1
            return
    ...
    def read_base(self, cnt, max_hold, reset_zero=True):
        ...
        for i in range(3):
            ...
            t_last = time.time()
            while not (
                    self.is_shutdown or self.is_timeout) and len(
                    self.get_vals()[0]) < cnt and (
                    time.time() - t_last) < cnt * 0.010 * 15:
                self.delay_s(0.010)
                pass
            vals = self.get_vals()
            if len(vals[0]) < cnt:
